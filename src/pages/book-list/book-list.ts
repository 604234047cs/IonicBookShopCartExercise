...
export class BookListPage {
  ...
  BookAdd(bookId:number,title:string,price:number) {      
    var ShopCart:any[];     var numItem;     var total; 
    if (localStorage.getItem('ShopCart')==null || (localStorage.getItem('ShopCart')==='undefined')){                
      ...
      ShopCart.filter(book => ( book.bookid === bookId))[0].quatity=ShopCart.filter(book => ( book.bookid === bookId))[0].quatity+1;               }            localStorage.setItem('ShopCart', JSON.stringify(ShopCart));            }                       numItem=Number(localStorage.getItem('numItem'));     numItem=numItem+1;     localStorage.setItem('numItem', numItem);     this.numItem=Number(numItem); 
